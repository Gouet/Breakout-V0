...
class ActorCritic():
    def __init__(self, num_action, lr, clip_param, final_step, state_shape, filename='./saved/actor.h5'):
        ...
        x = tf.keras.layers.Conv2D(32, 8, 4, activation=tf.nn.relu, kernel_initializer=w)(self.input)
        x = tf.keras.layers.Conv2D(64, 4, 2, activation=tf.nn.relu, kernel_initializer=w)(x)
        x = tf.keras.layers.Conv2D(64, 3, 1, activation=tf.nn.relu, kernel_initializer=w)(x)
        x = tf.keras.layers.Flatten()(x)
        x = tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer=w)(x)
